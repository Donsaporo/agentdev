/*
  ...
  1. New Tables
    - `team_members` - Obzide team members (linked to auth.users)
      ...
      - `full_name` (text)
      - `role` (text) - admin, developer, manager
      - `avatar_url` (text)
    ...
    - `clients` - Client information
      ...
      - `contact_phone` (text)
      - `industry` (text)
      - `brand_colors` (jsonb) - palette from brand manual
      - `brand_fonts` (jsonb) - typography info
      - `notes` (text)
    ...
    - `projects` - Development projects
      ...
      - `name` (text)
      - `type` (text) - website, ecommerce, mobile_app, crm, custom
      - `status` (text) - draft, planning, in_progress, qa, review, approved, deployed
      - `description` (text)
      - `demo_url` (text) - subdomain on obzide.com
      - `production_url` (text) - final client domain
    ...
    - `briefs` - Client briefs / requirements
      ...
      - `original_content` (text) - raw brief text
      - `parsed_requirements` (jsonb) - structured requirements
      - `pages_screens` (jsonb) - list of pages/screens
      - `features` (jsonb) - required features
      - `questions` (jsonb) - agent questions
      - `answers` (jsonb) - team answers
      - `status` (text) - pending_review, questions_pending, approved, in_progress
      - `architecture_plan` (jsonb) - generated architecture
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)
    ...
    - `integrations` - API integrations per project
      - `id` (uuid, primary key)
      - `project_id` (uuid, references projects)
      - `service_name` (text) - resend, stripe, namecheap, etc
      - `service_type` (text) - payment, email, dns, crm, custom
      - `config` (jsonb) - non-sensitive configuration
      - `status` (text) - configured, active, error
    ...
    - `agent_logs` - Activity log from the AI agent
      ...
      - `category` (text) - development, deployment, dns, qa, error
    ...
    - `domains` - Domain management
      ...
      - `client_id` (uuid, references clients)
      - `domain_name` (text)
      - `subdomain` (text) - e.g. "lintonbay" for lintonbay.obzide.com
      - `is_demo` (boolean) - true for obzide.com subdomains
      - `dns_status` (text) - pending, configured, verified, error
      - `ssl_status` (text) - pending, active, error
      - `registrar` (text) - namecheap, etc
      - `nameservers` (jsonb)
      - `created_at` (timestamptz)
      - `updated_at` (timestamptz)
  ...
  2. Security
    ...
    - Team members can only access data they created or are assigned to
