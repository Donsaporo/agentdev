...
export default function QAScreenshotCard({ screenshot, onApprove, onReject, onPreview }: QAScreenshotCardProps) {
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-800/60 rounded-xl overflow-hidden hover:border-slate-700/60 transition-all">
      ...
      <div className="grid grid-cols-3 gap-px bg-slate-800/30">
        {devices.map(device => (
          <div key={device.key} className="bg-slate-900/80 p-3">
            ...
            {device.url ? (
              <button
                ...
                className="group relative w-full aspect-[4/3] bg-slate-800 rounded-lg overflow-hidden border border-slate-700/30"
              >
                <img
                  src={device.url}
              ...
              <div className="w-full aspect-[4/3] bg-slate-800 rounded-lg border border-slate-700/30 flex items-center justify-center">
          ...
          {showRejectForm ? (
            <div className="space-y-2">
              <textarea
                ...
                rows={2}
                className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-amber-500/50 resize-none"
              />
              <div className="flex items-center gap-2">
                <button
                  onClick={handleReject}
                  disabled={!rejectionNotes.trim()}
                  className="px-3 py-1.5 bg-amber-500/20 text-amber-400 text-xs font-medium rounded-lg hover:bg-amber-500/30 transition-colors disabled:opacity-40"
                >
                  Submit Fix Request
                </button>
                <button
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-500/10 text-emerald-400 text-xs font-medium rounded-lg hover:bg-emerald-500/20 transition-colors"
              >
                <Check className="w-3.5 h-3.5" /> Approve
              </button>
              <button
                onClick={() => setShowRejectForm(true)}
                className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-amber-500/10 text-amber-400 text-xs font-medium rounded-lg hover:bg-amber-500/20 transition-colors"
              >
